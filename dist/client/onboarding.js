...
function page(baseUrl, state, message) {
...
export class GreenweezOnboarding {
    ...
    token;
    state = "ready";
    message = "Choisissez la connexion ou la création de compte.";
    loginPromise;
    closeTimer;
    constructor(gateway) {
    ...
    url() {
        ...
        if (!this.token || !address || typeof address === "string")
            throw new ConfigurationError("Le wizard Greenweez local ne peut pas déterminer son adresse.", "Relancez l’outil connect_greenweez.");
    ...
    async handle(requestUrl, response) {
        this.resetCloseTimer();
        ...
        try {
            ...
            this.send(response, 400, "Requête invalide.", "text/plain; charset=utf-8");
            return;
        }
        if (!url.pathname.startsWith(`/${this.token}`)) {
            this.send(response, 404, "Introuvable.", "text/plain; charset=utf-8");
            return;
        }
    ...
    startLogin() {
        if (this.loginPromise || this.state === "connected")
            return;
    ...
    async begin() {
        ...
        if (status.connected)
            return { status: "connected", officialLinks, message: "La session Greenweez est déjà connectée et vérifiée." };
