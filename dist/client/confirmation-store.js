...
function paths(environment) {
    ...
    if (!/^[a-z0-9][a-z0-9._-]{0,63}$/i.test(scope))
        throw new ConfigurationError("GREENWEEZ_CAMOFOX_USER_ID est invalide pour le registre de confirmations.", "Utilisez un identifiant local stable composé de lettres, chiffres, points, tirets ou underscores.");
...
function acquire(path, purpose) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
            ...
            if (code === "EEXIST")
                throw new ConflictError(`Une autre ${purpose} Greenweez est déjà en cours.`, "Attendez sa fin puis relisez le panier avant de recommencer.");
...
export class ConfirmationStore {
    ...
    load() {
        ...
        try {
            ...
            throw new ContractChangedError("Le registre privé des confirmations Greenweez est illisible.");
        }
        const parsed = fileSchema.safeParse(value);
        if (!parsed.success)
            throw new ContractChangedError("Le registre privé des confirmations Greenweez a un format inconnu.");
        if (process.platform !== "win32")
            chmodSync(this.file, 0o600);
        return parsed.data;
    }
    create(pending, ttlMs = 120_000) {
        ...
        try {
            ...
            const now = Date.now();
            for (const [digest, item] of Object.entries(state.pending))
                if (Date.parse(item.expiresAt) <= now)
                    delete state.pending[digest];
    ...
    take(token) {
        if (!/^[A-Za-z0-9_-]{43}$/.test(token))
            throw new ConflictError("Le jeton de confirmation Greenweez est invalide.", "Relancez la prévisualisation et utilisez exactement le nouveau jeton retourné.");
        ...
        try {
            ...
            atomicWrite(this.file, state);
            if (!pending)
                throw new ConflictError("Cette confirmation Greenweez est inconnue ou a déjà été utilisée.", "Relancez la prévisualisation sur l’état actuel du panier.");
            if (Date.parse(pending.expiresAt) <= Date.now())
                throw new ConflictError("Cette confirmation Greenweez a expiré.", "Relancez la prévisualisation sur l’état actuel du panier.");
            return pending;
        }
        finally {
            release();
