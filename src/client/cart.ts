...
function browserGraphqlPreamble(): string {
  return String.raw`
...
function mutationExpression(kind: "add" | "remove", id: number, expectedCartTokenHash: string): string {
...
function offerExpression(reference: string): string {
  return String.raw`(() => {
...
function stateVersion(cart: RawCart): string {
...
function tokenHash(cart: RawCart): string {
  return createHash("sha256").update(cart.tokenValue, "utf8").digest("hex");
}

function publicCart(cart: RawCart): PublicCart {
  return {
...
function quantity(cart: RawCart, reference: string): number {
...
export class GreenweezCartClient {
  constructor(private readonly browser: BrowserGateway, private readonly confirmations = new ConfirmationStore()) {}

  private async rawCart(): Promise<RawCart> {
    const parsed = cartEvaluationSchema.safeParse(await this.browser.read(GREENWEEZ_HOME, cartExpression()));
    if (!parsed.success) throw new ContractChangedError("La réponse minimale du panier Greenweez ne correspond plus au contrat observé.");
    if (!parsed.data.ok) {
      if (parsed.data.reason === "missing_or_expired_session") throw new ConnectionError("La session Greenweez est absente ou expirée.", "Importez un bundle de session valide ou reconnectez-vous localement, puis réessayez.");
  ...
  async getCart(): Promise<PublicCart> {
    return publicCart(await this.rawCart());
  }

  async previewAdd(product: ProductForCart): Promise<AddPreview> {
    const rawOffer = offerSchema.safeParse(await this.browser.read(new URL(product.url), offerExpression(product.reference)));
    if (!rawOffer.success) throw new ContractChangedError("La fiche produit Greenweez ne fournit plus l’offre avec le contrat observé.");
    if (!rawOffer.data.ok) throw new ContractChangedError(`L’offre Greenweez ne peut pas être résolue (${rawOffer.data.reason}).`);
    const cart = await this.rawCart();
    const before = quantity(cart, product.reference);
    if (!rawOffer.data.inStock || rawOffer.data.quantityAvailable <= before) throw new ConflictError("Ce produit ne dispose pas d’une unité supplémentaire vérifiée.", "Relisez la fiche produit ou choisissez un autre article disponible.");
  ...
  async previewRemove(reference: string): Promise<RemovePreview> {
    const cart = await this.rawCart();
    const matches = cart.items.filter(item => item.offer.variant.code === reference);
    if (matches.length !== 1) throw new ConflictError(matches.length ? "Plusieurs lignes correspondent à cette référence." : "Cette référence n’est pas présente dans le panier.", "Relisez le panier et ciblez une référence présente sur une seule ligne.");
  ...
  async confirm(token: string, expectedKind: "add" | "remove"): Promise<MutationResult> {
    return this.confirmations.withMutationLock(async () => {
      const pending = this.confirmations.take(token);
      if (pending.kind !== expectedKind) throw new ConflictError("Le jeton de confirmation ne correspond pas à cette action.", "Utilisez l’outil de confirmation indiqué par la prévisualisation, ou relancez celle-ci.");
      const before = await this.rawCart();
      if (stateVersion(before) !== pending.stateVersion) throw new ConflictError("Le panier Greenweez a changé depuis la prévisualisation.", "Relisez le panier et créez une nouvelle prévisualisation ; aucune mutation n’a été envoyée.");
      ...
      if (!verified) {
        if (mutation.success && !mutation.data.ok && mutation.data.reason === "cart_token_changed") throw new ConflictError("Le jeton du panier a changé avant la mutation.", "Relisez le panier et créez une nouvelle prévisualisation ; aucune mutation n’a été appliquée.");
