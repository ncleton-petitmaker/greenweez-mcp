...
import { GreenweezCartClient, type AddPreview, type MutationResult, type PublicCart, type RemovePreview } from "./cart.js";
...
function parse<T>(schema: z.ZodType<T>, value: unknown, operation: string): T {
  ...
  if (!parsed.success) throw new ContractChangedError(`Le contrat HTML Greenweez a changé pendant ${operation} : ${parsed.error.issues[0]?.message ?? 'données invalides'}.`);
  return parsed.data;
}

export class GreenweezClient {
  private readonly cartClient: GreenweezCartClient;

  constructor(private readonly browser: BrowserGateway, confirmations = new ConfirmationStore()) {
  ...
  async search(query: string, page: number): Promise<SearchResult> {
    ...
    if (!extracted.query || extracted.query.localeCompare(normalized, "fr", { sensitivity: "base" }) !== 0) throw new ContractChangedError("La page retournée ne correspond pas à la recherche demandée.");
    return { ...extracted, query: normalized, page };
  }

  async product(reference: string, slug: string): Promise<ProductDetail> {
    const validReference = /^[A-Z0-9]+$/i.test(reference);
    const validSlug = /^[a-z0-9]+(?:-[a-z0-9]+)*$/i.test(slug);
    if (!validReference || !validSlug) throw new ContractChangedError("La référence ou le slug produit est invalide.");
    ...
    if (product.reference !== reference) throw new ContractChangedError("La fiche produit retournée ne correspond pas à la référence demandée.");
    return product;
  }

  async cart(): Promise<PublicCart> {
    return this.cartClient.getCart();
  }

  async previewAddToCart(reference: string, slug: string): Promise<AddPreview> {
  ...
  async previewRemoveFromCart(reference: string): Promise<RemovePreview> {
    return this.cartClient.previewRemove(reference);
  }

  async confirmAddToCart(confirmationToken: string): Promise<MutationResult> {
    return this.cartClient.confirm(confirmationToken, "add");
  }

  async confirmRemoveFromCart(confirmationToken: string): Promise<MutationResult> {
