...
test("MCP SDK exposes read-only product search", async () => {
  ...
  assert.deepEqual(tools.tools.map((tool) => tool.name).sort(), ["confirm_add_to_cart", "confirm_remove_from_cart", "get_cart", "get_product", "preview_add_to_cart", "preview_remove_from_cart", "search_products"]);
